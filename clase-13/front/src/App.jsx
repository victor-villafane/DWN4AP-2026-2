...
const App = () => {
  ...
  const tacharPersonaje = (indice) => {
    const personajesCopia = [...personajes]
    personajesCopia[indice].completado = !personajesCopia[indice].completado
    setPersonajes(personajesCopia)
  }
        ...
        <tbody>
          { // v-for="(personaje, indice) in personajes"
            personajes.map((personaje, indice) => (
              <tr
                className={`hover:bg-gray-800/50 ${personaje.completado ? "line-through" : ""}`}
                key={indice}
              >
                ...
                <td className="border border-gray-700 p-3" >
                  <button
                    className="rounded bg-amber-300 px-4 py-1.5 mx-2 text-sm"
                    onClick={() => tacharPersonaje(indice)}
                  >
                    Completado
                  </button>
                  <button
                    className="rounded bg-red-600 px-4 py-1.5 mx-2 text-sm"
