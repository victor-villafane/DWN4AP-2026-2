...
export function createListPage(lista) {
  ...
  `
    ...
    <tr>
      ...
      <td>${juego.genres}</td>
      <td class="d-flex" >
        <a class="btn btn-secondary mx-1" href="/juegos/${juego.appid}" >Ver</a>
        <a class="btn btn-warning mx-1" href="/juegos/editar/${juego.appid}" >Editar</a>
        <a class="btn btn-danger mx-2" href="/juegos/borrar/${juego.appid}" >Borrar</a>
      </td>
    </tr>` )
    html += "</tbody></table>"
    return html
...
export function createGameFormEditPage(juego) {
    let html = ""
    html += `<form action='/juegos/editar/${juego.appid}' method='POST' >`
    html += `
        <div class="my-2" >
            <label class="form-label" >Nombre: </label>
...
export function createDetailDelete(juego) {
    let html = `<form action='/juegos/borrar/${juego.appid}' method="POST" >`
    html += `<p>Developer: ${juego.developer}</p>`
    html += `<p>Publisher: ${juego.publisher}</p>`
    html += `<p>Platforms: ${juego.platforms}</p>`
    html += `<p>Categories: ${juego.categories}</p>`
